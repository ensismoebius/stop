...
// Re-difusao de confirmacao apos a transicao para PLAYING (tempo-real.md #4).
...
export async function beginPlaying(roundId) {
  return gameLock.run(lockKey(roundId), async () => {
    ...
    await broadcastState(room.code);
    // Reforco da transicao (tempo-real.md #4): PLAYING e o estado que libera as
    ...
    // (tempo-real.md #1), entao isto e so a primeira linha de defesa.
